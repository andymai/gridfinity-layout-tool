...
| `magnetCrushRibs` | `boolean`                                                                                              |          |              |             | Crush ribs inside each magnet hole: a wavy bore that grips a press-fit magnet without glue. Absent means off.                                           |
| `magnetChamfer`   | `boolean`                                                                                              |          |              |             | 45 degree lead-in at each magnet hole's mouth. Skipped where the surrounding wall is too thin. Absent means off.                                        |
...
| `lipTip`          | `"sharp"` \| `"round"` \| `"chamfer"`                                                                  |          | `"sharp"`    |             | Finish on the stacking lip's peak. Rounding or chamfering it gives the top layer a printable perimeter instead of a knife edge; stacking is unaffected. |
...
| `floorRaises`            | `number` \| `null`[]                          |          |         |                | Per-compartment floor raise in mm, indexed by compartment id. Lifts that compartment's floor on a solid slab so short items sit nearer the rim; null or omitted stays on the standard floor. |
...
| `openSides`        | [`CutoutOpenSideSpec`](#cutoutopensidespec)[]                                                                                        |          |                   | items <= 4          | Bin walls the pocket opens through so a part slides in from the side. Any profile shape on a solid, untapered bin; a leaned pocket or a repeated group stays enclosed. Omit for enclosed.   |
...
One wall a pocket opens through. Write `{ "side": "right" }` for a channel the
pocket's own width, add `widthMm` for a narrower keyhole exit, and `tunnel`
to keep the wall above the pocket.

<!-- schema:CutoutOpenSideSpec indexed -->

<a id="cutoutopensidespec"></a>

3 fields, in `src/features/bin-designer/types/cutout.ts`.
