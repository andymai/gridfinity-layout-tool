...
| `lipTip`          | `"sharp"` \| `"round"` \| `"chamfer"` \| `"flat"`                                                      |          | `"sharp"`    |             | Finish on the stacking lip's peak. Rounding, chamfering or flattening it gives the top layer a printable perimeter instead of a knife edge; stacking is unaffected. |
...
| `openSides`        | `any`[]                                                                                                                              |          |                   | items <= 4          | Bin walls the pocket opens through so a part slides in from the side. Any profile shape on a solid, untapered bin; a leaned pocket or a repeated group stays enclosed. Omit for enclosed.   |
