...
| `openSides`        | `"front"` \| `"back"` \| `"left"` \| `"right"`[]                                                                                     |          |                   | items <= 4          | Bin walls a rectangle pocket opens through, floor to rim, so a part slides in from the side. Only built on a solid, untapered bin for an ungrouped upright rectangle rotated in 90° steps; otherwise the pocket stays enclosed. Omit for enclosed. |
