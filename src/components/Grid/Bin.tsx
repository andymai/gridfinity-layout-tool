...
function BinComponent({ bin, category, layer, drawer, cellSize, gap = 1, halfBinMode: _halfBinMode = false, isGhost, isSelected, onStartDrag, onStartResize }: BinProps) {
  ...
  return (
    ...
    >
      ...
      {/* Metadata indicator pill - shown when bin has notes or custom properties */}
      {hasMetadata && showAnyText && !isGhost && (
        <div
          className="absolute bottom-0.5 left-0.5 flex items-center gap-0.5 px-1 py-0.5 rounded-full pointer-events-none bg-surface/90 backdrop-blur-sm"
        ...
        >
          ...
          {hasNotes && (
            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} style={{ color: 'rgb(96, 165, 250)' }}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
            </svg>
          )}
          {/* Custom properties indicator - tag icon */}
          {hasCustomProps && bin.customProperties && (
            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} style={{ color: 'rgb(251, 191, 36)' }}>
