 ...
 *
 * Identical to `Partial<Bin>` except that `overhang`, `pairId`, `pairRole` and
 * `linkedDesignId` are tri-state: omitted leaves the field alone, a value sets
 * it, and `null` clears it. A plain optional field can't express "clear" —
 * `{ linkedDesignId: undefined }` is indistinguishable from omission once it
