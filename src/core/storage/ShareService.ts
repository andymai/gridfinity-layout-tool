 ...
 * Matches: /l/{id} or /l/{id}/{slug} where id is one of the formats the
 * server accepts (12-char alphanumeric, legacy UUID). Kept in sync with
 * `parseLayoutFromURL` in `@/shared/utils/url` and the server's
 * `isValidShareId` — divergence silently breaks share-link viewing.
 */
