import { useState, useCallback } from 'react';
import { Button } from '@/design-system';
import { DeferredNumberInput } from '@/shared/components/DeferredNumberInput';

export interface LinkedDimensionInputProps {
  /** Primary-axis value in mm */
  width: number;
  /** Secondary-axis value in mm (pass the effective value: stored ?? width) */
  depth: number;
  /** Called with (width, depth?) — depth is undefined when linked (square) */
  onChange: (width: number, depth?: number) => void;
  /** Optional id forwarded to the first input (for htmlFor label association) */
  id?: string;
  /** 'compact' = sidebar/defaults, 'mobile' = mobile panel */
  variant?: 'compact' | 'mobile';
  min: number;
  max: number;
  step?: number;
  widthAriaLabel: string;
  depthAriaLabel: string;
  linkAriaLabel: string;
  unlinkAriaLabel: string;
}

const LINK_ICON =
  'M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71';
const UNLINK_ICON_PATHS = [
  'M18.84 12.25l1.72-1.71a5 5 0 0 0-7.07-7.07l-1.72 1.71',
  'M5.17 11.75l-1.72 1.71a5 5 0 0 0 7.07 7.07l1.72-1.71',
  'M8 21l1-3M16 3l-1 3M21 8l-3 1M3 16l3-1',
];

/**
 * A square-by-default dimension pair: one input plus a link icon while both
 * axes match; unlinking splits it into width × depth inputs. Relinking
 * collapses back to square (`onChange(width)` with no depth).
 */
export function LinkedDimensionInput({
  width,
  depth,
  onChange,
  id,
  variant = 'compact',
  min,
  max,
  step = 1,
  widthAriaLabel,
  depthAriaLabel,
  linkAriaLabel,
  unlinkAriaLabel,
}: LinkedDimensionInputProps) {
  // Track whether the user has expanded to show two inputs.
  // When width !== depth, always show two inputs regardless.
  const [expanded, setExpanded] = useState(false);
  const isSquare = width === depth;
  const showSingleInput = isSquare && !expanded;

  const handleWidthChange = useCallback(
    (value: number) => {
      onChange(value, depth !== value ? depth : undefined);
    },
    [onChange, depth]
  );

  const handleDepthChange = useCallback(
    (value: number) => {
      onChange(width, value !== width ? value : undefined);
    },
    [onChange, width]
  );

  const handleToggleLink = useCallback(() => {
    if (showSingleInput) {
      setExpanded(true);
    } else {
      setExpanded(false);
      onChange(width);
    }
  }, [showSingleInput, width, onChange]);

  const isCompact = variant === 'compact';
  const inputClass = isCompact
    ? 'input w-14 py-0.5 px-1 text-xs text-right'
    : 'input w-20 h-10 text-center';
  const iconSize = isCompact ? 'w-3 h-3' : 'w-4 h-4';
  const btnClass = `flex-shrink-0 rounded border border-stroke-subtle text-content-secondary hover:text-content hover:bg-surface-hover hover:border-stroke transition-colors focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-accent ${isCompact ? 'p-0.5' : 'p-1'}`;

  const linkButton = (
    <Button
      type="button"
      variant="ghost"
      touchTarget={false}
      onClick={handleToggleLink}
      className={btnClass}
      aria-label={showSingleInput ? unlinkAriaLabel : linkAriaLabel}
    >
      <svg
        className={iconSize}
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth={2}
        strokeLinecap="round"
        strokeLinejoin="round"
      >
        {showSingleInput ? (
          <path d={LINK_ICON} />
        ) : (
          UNLINK_ICON_PATHS.map((d) => <path key={d} d={d} />)
        )}
      </svg>
    </Button>
  );

  if (showSingleInput) {
    return (
      <div className="flex items-center gap-1">
        <DeferredNumberInput
          id={id}
          value={width}
          onChange={onChange}
          min={min}
          max={max}
          step={step}
          className={inputClass}
          aria-label={widthAriaLabel}
        />
        {linkButton}
      </div>
    );
  }

  return (
    <div className="flex items-center gap-1">
      <DeferredNumberInput
        id={id}
        value={width}
        onChange={handleWidthChange}
        min={min}
        max={max}
        step={step}
        className={inputClass}
        aria-label={widthAriaLabel}
      />
      <span className="text-micro text-content-tertiary">×</span>
      <DeferredNumberInput
        value={depth}
        onChange={handleDepthChange}
        min={min}
        max={max}
        step={step}
        className={inputClass}
        aria-label={depthAriaLabel}
      />
      {linkButton}
    </div>
  );
}
