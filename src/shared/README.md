...
| `binSplitFit.ts`           | `binSplitMaxGridUnits()` — grid capacity of one build plate with the bin's OVERHANG charged against it. `calcMaxGridUnits` measures the nominal footprint, which a bin can overrun by millimetres without gaining a unit                                        |
