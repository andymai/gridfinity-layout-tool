...
11. **Lid/bin magnet seat planes** — the lid's boss and the bin's gusset pad are built by different passes and must meet across exactly one `LID_MAGNET_SEAT_GAP` (0.2mm). Both derive from `retentionSeatPlanes()` — the Z counterpart of `retentionMagnetPositions` for XY — so neither side can drift. **Anything that moves the lid in Z must be checked against the boss's reach**: a deep cavity lifts the bosses clear of the bin's rim, at which point the bin would grow pads floating above its own lip (`magnetLidCavityTooDeep` blocks it upstream). Note there are four independent expressions of the seating transform (`exportHandler`, `lidRetentionStage`, `LidMesh`, `thumbnailRegenerator`); the scenario suite pins the first two together.
