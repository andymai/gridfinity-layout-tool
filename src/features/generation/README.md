...
- `worker/generators/stackingFloorBuilder.ts` — bed-supported floor for the Stacking body. Called inside `trayBottomStage` before retention pockets are drilled, so the floor cannot cap the magnet openings. Drainage tools run after the replacement slab and preserve retention-boss and divider-foot keep-outs. Keeps divider supports down to the slab and reuses the lid's mating profile and magnet positions. `stackingFloor.geometry.test.ts` checks floor height, pocket openings, watertightness, and sampled stack interference for the 6×5×12u case, including deep magnets.
- `worker/generators/stackingMagnetBoss.ts` — Stacking-only corner supports with straight wall tangents and a rounded inward corner. Clips the wall-facing footprint to the mating shell; ordinary lids retain cylindrical bosses.
