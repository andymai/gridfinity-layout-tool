...
9. **Wall key tongue/groove must stay matched** — in `splitConnectorBuilder.ts` the male tongue (`inflate=0`) and the female groove (`inflate=clearance`) are both built by `buildKey` from the _same_ `keyHeight` and `WallKeyGeometry`, so the groove is exactly the tongue grown by the fit clearance. `buildKey` clamps `WALL_KEY_PROTRUSION` to the available `keyHeight` so the tongue's 45° self-supporting tip ramp always finishes below the lead-in notch — the clamp is deterministic from `keyHeight`, which is why both halves stay matched. **Raising `WALL_KEY_PROTRUSION`/`WALL_KEY_HALF_WIDTH` deepens the groove inward (`pilasterPerpDepth`), growing the reinforcing pilaster onto more wall thicknesses; verify the new size still leaves a positive outer skin and passes `binGenerator.scenario` watertightness.**
