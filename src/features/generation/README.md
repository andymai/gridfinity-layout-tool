...
7. **Lid overhang uses two coordinate families** — when a bin has an overhang the lid grows to match (see `lidInputs.ts` `resolveLidInputs`), but its features split into two frames. _Perimeter-anchored_ geometry (outer outline, mating shell, floor, click rails) follows the overhang-expanded/shifted body via `outerOffsetX/Y` on `LidInputs` — all of it routes through `buildOutlineDrawing` (perimeter) or the shifted rail placements. _Grid-anchored_ features (stack-grid pockets, the lip-only stack pocket, magnet holes, and lid text when it lands on the lip-only floor) stay pinned to the nominal socket grid at the origin, because overhang leaves the base sockets at nominal footprint so a stacked bin still mates on the standard gridfinity grid. Note that grid-anchored geometry can't reuse `buildOutlineDrawing` even at a negative inset — that outline is both shifted _and_ grown by the overhang; `lidStackGrid.ts` `buildStackLipCutter` sizes itself from `cellsX × gridUnitMm` instead. **Any new lid feature must pick the right frame**: add the offset for perimeter-following geometry, omit it for anything that must align to the socket grid. Overhang is suppressed for polygon (`cellMask`) lids, mirroring `boxBuilder`.
