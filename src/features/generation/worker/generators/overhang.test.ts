...
describe('resolveOverhang', () => {
  it('returns all-zero for undefined', () => {
    expect(resolveOverhang(undefined)).toEqual({ left: 0, right: 0, front: 0, back: 0 });
  });
...
describe('hasOverhang', () => {
  it('is false for all-zero', () => {
    expect(hasOverhang({ left: 0, right: 0, front: 0, back: 0 })).toBe(false);
  });
  it('is true when any side is positive', () => {
    expect(hasOverhang({ left: 0, right: 0, front: 0, back: 0.5 })).toBe(true);
  });
});

describe('overhangExpansion', () => {
  it('sums opposite sides and centers symmetric overhang', () => {
    const e = overhangExpansion({ left: 5, right: 5, front: 4, back: 4 });
  ...
  it('offsets the center toward the larger side for asymmetric overhang', () => {
    const e = overhangExpansion({ left: 0, right: 6, front: 2, back: 0 });
...
describe('overhangKey', () => {
  it('is a stable "0" when there is no overhang', () => {
    expect(overhangKey({ left: 0, right: 0, front: 0, back: 0 })).toBe('0');
  });
  it('distinguishes different overhang configs', () => {
    const a = overhangKey({ left: 1, right: 2, front: 3, back: 4 });
    const b = overhangKey({ left: 4, right: 3, front: 2, back: 1 });
    expect(a).not.toBe(b);
  });
});
