...
export function SharedWithMeList({ onOpenLayout }: SharedWithMeListProps) {
  ...
  const sortedEntries = [...sharedWithMe].sort((a, b) => b.lastAccessedAt - a.lastAccessedAt);

  // Derive valid focused index (clamp to list bounds)
  const validFocusedIndex = Math.min(focusedIndex, Math.max(0, sortedEntries.length - 1));
