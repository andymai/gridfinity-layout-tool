...
export function useGridCoords(gridRef: RefObject<HTMLDivElement | null>) {
    ...
    (clientX: number, clientY: number): Coord | null => {
      ...
      if (halfGridMode) {
        ...
        if (cellX === -1) {
          ...
          const baseX =
            hasFractionalWidth && fractionalEdgeX === 'start' ? fractionalWidthPart + cellX : cellX;
        ...
        if (cellY === -1) {
          ...
          // In integer row
          const baseY =
            hasFractionalDepth && fractionalEdgeY === 'end'
  ...
  const isInBounds = useCallback(
    (coord: Coord): boolean => {
      return coord.x >= 0 && coord.x < drawer.width && coord.y >= 0 && coord.y < drawer.depth;
    },
