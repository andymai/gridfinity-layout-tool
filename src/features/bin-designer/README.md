...
- **Click-lock lid**: optional companion piece generated alongside the bin
  ...
  XY only) so the magnets aren't fighting a friction fit. Magnetic retention
  additionally requires the lid's boss to reach a pad the bin can host — a deep
  cavity lifts it clear of the rim, which `magnetLidCavityTooDeep` blocks (a
  deeper magnet extends the reach and unblocks it).
