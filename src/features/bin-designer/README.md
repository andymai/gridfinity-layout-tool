...
- **Lid-compatible bottom (`base.style: 'lid'`)**: a fully editable bin whose underside is lid mating geometry instead of a Gridfinity socket, so a shallow organiser caps the bin below it. `base.trayBottom` is a narrow subset of `LidConfig` sharing field names and units; the lid's top-face fields are omitted because a tray's top is its own interior.
  - **Stacking body** opts into `base.trayBottom.floorAtBed: true`. It reuses the lid mating taper, fills the plug, and opens the original body floor down to a bed-supported slab. Magnet bosses and divider supports stay solid. `resolveTrayBottomConfig` fixes clearance at zero, excludes click rails, and shares the lid's magnet dimensions with four bottom pockets. This stacks on same-size bins, not Gridfinity baseplates. Existing lid-base designs keep their raised floor. Floor-mounted inserts, scoops, drainage patterns, and removable dividers are unavailable in this mode because they use the original floor datum.
