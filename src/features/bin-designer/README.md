...
- `components/panel/HingeControls/` — the hinged lid's knobs: which wall the axis runs along, and what holds the lid SHUT (nothing / a snap rail on the opposite wall / magnet bosses on that same wall). A hinge fixes one edge and says nothing about the other three, which is why the catch is its own choice rather than a fifth attachment. The readout is the point: the pin is a filament offcut the user cuts themselves, so the cut length is the one number nobody can measure off the model, and it is quoted again in the export dialog from the same `hingePinLengths` call — one per RUN, because a cutout that splits the hinge wall leaves two barrels and needs two pins. The running fit is deliberately NOT here; it lives under Advanced with the other millimetre knobs, on the same argument as the sliding clearance. Switching TO a hinge seeds a scallop grip on the opposite wall when the grip is still untouched, since the default grip is `none` and a hinged lid with nothing to lift is a bad first print — but it never overwrites a configured one
...
- `components/preview/HingePin/` — the filament pin, drawn on the hinge axis in a neutral hardware grey so it reads as the offcut it is rather than something the printer produces. Rendered OUTSIDE the lid's rotating group, because the pin stays on the axis at every opening angle; without it the interleaved knuckles are two rows of half-cylinders and what they are for is not obvious. `lidHingePose` in `preview/LidMesh/lidAnchorZ.ts` owns the pose itself, as three nested groups rather than one — a group turns about its own origin and the axis is out at the wall, so collapsing them looks identical at 0° and swings the lid through the bin at anything else
...
- **Spacer / riser (`base.spacer`, issue #2869)**: a floorless bin used as a riser so bins of different heights finish flush. Deliberately NOT a new `BinStyle` — it's a base property, so it composes with the wall features and doesn't ripple through the ~20 `style` consumers. Implementation is almost entirely reuse: it derives `dimensions.lightweight = true` and asks `lightweightBaseBuilder` for the new `'through'` open direction, which is simply **zShift 0** — the inner foot is a uniform-`wallThickness` lateral offset at every depth, so an unshifted cut leaves the cup open at BOTH ends. The floor-opening slug the lite path already builds punches the body floor over each cup, so the whole cell becomes a through-hole. **What holds a multi-cell spacer together is the inter-cell webbing** (each cup keeps its own wall, and adjacent cups' walls meet) plus the perimeter — remove that and interior feet would come loose as separate solids, which is what the 3×3 scenario guards. Envelope, feet and lip are untouched, so the stacking arithmetic is inherited from a normal bin: a 2u spacer under a 2u bin reaches the top of a 4u one. **A spacer is the one bin allowed down to 1u** (#2915) — `DESIGNER_CONSTRAINTS.MIN_HEIGHT` is 2 because 1u leaves no usable cavity, which a floorless riser doesn't have anyway, and 1u is what shims a stack from an odd to an even total height. The floor lives in ONE place, `minHeightUnits(base)`, read by the stepper bound, `validateBinParams` and every base toggle; `api/lib/designerValidation.ts` mirrors it (it cannot import from `src/`). It keys off the **effective** spacer — `base.spacer && style !== 'flat'`, the same condition `deriveDimensions` uses — because the flag is inert on a flat base, so keying off `base.spacer` alone would hand the relaxed floor to a `{ style: 'flat', spacer: true, height: 1 }` payload that generates an ordinary 1u bin. Because the constraint engine only enables and disables features, the numeric bound is not expressible as a rule: `useBaseSection` routes every toggle through one `commit()` that re-applies the floor. That has to cover more than the spacer card (`setBodyType`) — enabling the flat base auto-disables the spacer through `CONSTRAINT_RULES`, which would otherwise strand a 1u bin below the floor its own stepper enforces. `commit` reads the floor off the RESOLVED params, never the requested change, since the engine's post-check can return the params untouched when an enable turns out to be blocked. Layout placement is deliberately NOT included: `CONSTRAINTS.MIN_BIN_HEIGHT` stays 2, so a 1u spacer is a design/export part, not something you can drop into a drawer layout at its true height. **Attachment hardware is suppressed** (`deriveDimensions` zeroes `withMagnet`/`withScrew`, and the constraint engine disables the toggles): a magnet pad inside a through-hole would be a free-standing pillar. Incompatibilities are declared once in `CONSTRAINT_RULES`, and deliberately **one-way** from the interior features (the `style.solid` precedent): a spacer is a mode switch, so it stays reachable from a fully-designed bin and clears the incompatible set on the way in, rather than greying itself out and leaving the user to hand-clear compartments/scoop/label first. Only the flat base blocks it, and that block is genuinely mutual — a plate with no feet has nothing to open through. Every place that special-cases the shelled base takes the spacer too — the direct-mesh draft gate (`canBinUseDirectMesh`, or the instant preview would show a floor that isn't there), the split floor-scarf, the generation timeout budget, split alignment, and the floor-pattern fit/estimate
- **Underside lightweight relief (`base.lightweightMode`, issue #3524)**: the same lite shells opened DOWNWARD, so each foot becomes a ring and the bin's own floor caps it. The interior stays flat, which is the whole point: small or flat contents can't settle into the recesses, and the features that need a floor to stand on keep working. Not a new toggle — a mode on `base.lightweight`, optional and **absent** when interior so `communityParamsFingerprint` is unchanged for every already-published design (the `base.tile` precedent). Implementation is one open direction in `lightweightBaseBuilder`: **zShift 0 and no floor opening**, which is `'through'` minus the slug that punches the body floor. Cutting to exactly Z=0 rather than stopping a wall short of it IS the saving — the `'down'` direction solid bins already used leaves a `wallThickness` membrane under a floor that is already solid, costing a slab across the whole footprint without even shortening the bridge above it; `'down'` survives only as interior mode's answer for a solid bin, so designs saved before the mode still build what they were published with. **The offset is `UNDERSIDE_RELIEF_BORDER_MM` (3mm), not `wallThickness`**: this ring stands on the bed and carries a bridged floor, so it wants more material than a wall supported on both sides. It is safe at any value because the cut is a scaled copy of the socket profile — the insets are absolute, so an inner foot is a uniform wall at every depth. A straight prism would have to clear `SOCKET_TAPER_WIDTH` (3.2mm) before it left any wall at all, since a foot's bottom face is that much narrower than its cell; that is the trap the construction avoids by design. **`dimensions.lightweight` no longer answers the question the feature gates ask** — they want `liteFloorOpen` ("is the interior floor gone"), which is false for the relief, false for a solid bin whose body never had a distinct floor, and true for a spacer whether or not lite was requested. The relief therefore keeps the finger scoop, the floor pattern, top cutouts and the instant `canBinUseDirectMesh` draft (its change is invisible to the preview camera, exactly like magnet holes); floor inserts stay out of both modes, because a recess in a `wallThickness` floor with open air beneath does not thin it, it holes it. A **base-only bin** can now be lightweight too, in this mode only — it is feet plus one thin plate, so it gains most, and the old block was an accident of being bundled with the spacer rule. Two subtleties: the floor pattern's cut stops at the floor (`cutZ0`), because the relief cavity is already open to the outside and a full-depth tool would take up to 1.5mm off the inside of the ring's bottom face where the part meets the bed; and the divider clip (`openFloorDrawings`) is skipped, since a divider crossing a relieved foot already rests on continuous material. The mode selector nests inside the `FeatureToggle` like every other sub-control, which is safe only because the blocked case has its own way out: a "Use underside relief" action renders beside the greyed toggle whenever the hypothetical `underside` selection would lift the block (`undersideReliefUnblocks`), and `showFloor` includes that same flag so the family stays on screen when BOTH floor features are engine-disabled (a tile, or a solid board with cutouts) — otherwise the one control that can unblock the feature would be unmounted with it.
- **Lid-compatible bottom (`base.style: 'lid'`, issue #3036)**: an ordinary, fully editable bin whose underside is lid mating geometry instead of a Gridfinity socket, so a shallow organiser caps the bin below it. Chosen over extending the lid's own tray recess (#2694) because the reporter wanted the _whole_ editor — compartments, dividers, cutouts, labels — and a lid config cannot grow one without becoming a second editor. `base.trayBottom` is a **narrow subset** of `LidConfig` (attachment, `extraHeightMm`, click rails + coverage, retention magnet) sharing its field names and units so the two describe one joint; the lid's top-face fields are omitted because a tray's top is its own interior. The geometry is almost entirely reuse: `trayBottomInputs` synthesises a `lid` config and calls the real `resolveLidInputs`, and `trayBottomStage` fuses `buildMatingShell` + `addClickRails` + `addLidRetentionMagnets` under the body. **That reuse is only sound because the two profiles already coincide** — body outer is `w*grid - CLEARANCE` (0.5mm) and lid outer is `w*grid - 2*LID_FIT_CLEARANCE` (2×0.25mm), and both corner radii are 3.75mm (`BOX_CORNER_RADIUS` vs `LID_CORNER_RADIUS - fitClearance`) — so the skirt is flush with the wall and the fuse has no sliver. The watertight assertions in `trayBottomStage.geometry.test.ts` are what keep that claim honest if either constant moves. `resolveLidInputs` is fed `base.stackingLip: true` regardless of the tray's own lip: that field means "does the thing I mate with have a lip", and the answer is yes by construction since `shouldGenerateLid` refuses to build any lid for a lip-less bin. The tray's real `stackingLip` still governs its own top, so a Gridfinity bin can stack on it. `disabledRails` is forced empty — the rail-conflict check reads the COVERED bin's features, which live in a different design here, so deriving them from the tray's own walls would disable rails because of geometry on the wrong part. **Z arithmetic was generalised rather than special-cased**: `dimensions.baseOffsetZ` is the depth of whatever sits under the floor (`SOCKET_HEIGHT`, the skirt depth, or 0), and `translateStage`/`floorPatterns`/`meshImprint` read it instead of each re-deriving `isFlat ? 0 : SOCKET_HEIGHT`. Two traps worth knowing: `trayBottom` is deliberately **absent** from `DEFAULT_BIN_PARAMS` because `communityParamsFingerprint` hashes params wholesale, so an always-present new field would shift every published design's fingerprint and break the community duplicate guard; and `shellStage` gated the socket on `isFlat` alone, so tray bins grew feet AND a skirt while still passing every structural check — hence `dimensions.socketless` and the dimensional `sits on Z=0` test. `CLICK_RAIL_DROP_BELOW_WALL` ties the skirt depth to the rail profile, because rails hang below `wallBottomZ` and computing the lift from `wallBottomZ` alone sinks the model under the bed. Because a tray is a separate design, nothing structural links it to the bin it caps — `utils/matchingTray.ts` ("Create matching tray" in the lid section) is that link, copying footprint, grid pitch, fractional edges, polygon mask and attachment so a mismatch takes deliberate effort.
...
22. **Wall taper is stored rim-anchored but authored base-anchored** — the curved-bin taper (#2933) lives on `OverhangConfig.taper`: per-side inset from the rim, chamfer or fillet, shared band height. **What is stored is not what the panel shows.** `OverhangConfig.overhang` is the width at the rim and `taper` the inset back down; `OverhangSection` presents the drawer-measurable view instead — `base` fills the flat gap, `flare` adds width above it — deriving `base = overhang - taper` on read and writing back `overhang = base + flare`, `taper = flare`. Any new control here must write **both** fields in one `updateOverhang` call, or it silently moves the base the user set. Only an _enabled_ taper is subtracted: a dormant one keeps its per-side values for re-enabling while the stored overhang already reads as the base, which is also why toggling either way has to rewrite the four overhang sides. Flare does **not** require overhang on a side — `resolveTaper`'s `clamp(taper, overhang)` reads as "the base never drops below nominal", which the additive form satisfies by construction. Composes with **overhang feet**: `buildOverhangFeet` frames the feet from `overhangBaseSides` (the overhang minus the taper), not the stored rim values, so they stop where the tapered wall does — anything else that sits under the bin must do the same or it juts into open air. Available on every rectangular bin, hollow or solid: multi-compartment via the multi-cavity path's lofted outer (#3017), solid/cutout bins via the outer loft alone (#3033). Custom-shape (`cellMask`) bins are the one exception, and they are excluded further upstream — the whole overhang is suppressed for a partial mask, not just the taper. `paramSlice.updateOverhang` must carry `taper` through its field-by-field rebuild (clamped outward-only) — like the other overhang fields, it is dropped if omitted from the rebuild. The cutout editor draws the flare as a hatched band inset from each interior edge (`TaperBand3D`, sized by `cutoutTaperBand`): the placement frame stays rim-sized because that is genuinely usable at the fill surface, and only a pocket reaching the floor gets trimmed — so the band marks "deep cutouts stop here", not "do not draw here". Its width must come from `resolveOverhang` rather than `params.overhang.taper` directly, or it drifts from the clip on a side whose flare the generator clamps away. In the panel the flare repeats the overhang's four side names, so anything added there needs a distinct **accessible** name as well as a visual one (`OverhangSliderRow`'s `srLabel`, plus the `aria-labelledby` group) — otherwise two live sliders both answer to "Right".

23. **`Cutout.rotation` is clockwise-positive, every trig helper is counter-clockwise** — the stored angle is an SVG-era convention: `CutoutShapeMesh` renders at `rotationZ = -rotation` and `cutoutBuilder` extrudes at `rotate(shape, -rotation)`, while `rotatePoint` / `rotateAroundCenter` / `booleanGeometry`'s `rotatePair` are all standard CCW. **Anything that rotates a selection as a group must negate exactly one of the two** — the position swing or the per-member angle — or member centers travel one way while each member spins the other and the group shears instead of rotating (#3166). The live handle drag (`handlers/groupRotateHandler`) subtracts the drag delta from each member's rotation because positions rotate CCW with the cursor; the 90°/180°/numeric controls (`pathfinderHelpers.buildGroupRotationUpdates`) and the R key add to the rotation and rotate positions by `-deg`, which keeps a single selection turning the same visual direction it always has. A sign error here is invisible at 180° and on rectangles at 90° (both symmetric), so test rigidity at an odd angle with asymmetric shapes. The same trap bit the 2D preview: `booleanGeometry` must build outlines with the renderer's clockwise sense, or every rotated member gets a mirrored Pathfinder result and empty-result check.
