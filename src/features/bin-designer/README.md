...
15. **Split connectors have two joints** — `splitConnectors.enabled` adds a 45°
    floor scarf lap (always). `splitConnectors.wallConnector` (a
    `WallConnectorStyle`: `'none'` | `'key'`; sub-toggle in `SplitOptionsSection`,
    **default `'none'`**, #1869) adds an optional connector to the **exterior
    perimeter walls only**, reinforced by a full-height pilaster that thickens
    the wall **inward only** (preserving the Gridfinity footprint). The `'key'`
    style is a straight (non-undercut) tongue/groove so the halves **press
    ...
    untouched. **Adding a connector type:** extend `WallConnectorStyle`, add a
    `case` to the exhaustive `addWallConnectors` switch in
    `generation/worker/generators/splitConnectorBuilder.ts` (the compiler flags
    it until handled), reuse `perimeterWalls()` for placement, and add it to the
    UI. Off by default pending physical print validation.
