...
export function CutoutBoardSettings({
  ...
  return (
    <div className="space-y-3">
      <div>
        <span className="mb-1.5 block text-[10px] font-medium uppercase tracking-wide text-content-tertiary">
      ...
      {snapEnabled && (
        <div>
          ...
          <div role="group" aria-label={t('binDesigner.gridSize')} className={SEGMENT_GROUP_CLASS}>
            {GRID_SIZES.map((size) => (
              <button
                key={size}
                type="button"
                ...
                className={`flex-1 tabular-nums leading-none ${getSegmentClass(gridSize === size)}`}
              >
                {fmt(size)}
              </button>
            ))}
      ...
      <dl className="space-y-1 border-t border-stroke-subtle pt-2 text-[11px]">
