...
describe('a11y utilities', () => {
  describe('describeBin', () => {
    ...
    it('includes bin style when non-standard', () => {
      ...
      expect(describeBin(params)).toContain('slotted walls');
    });
    ...
    it('returns simple description when no features enabled', () => {
      const params: BinParams = {
        ...
        walls: {
          ...
          front: { ...DEFAULT_BIN_PARAMS.walls.front, width: 0, depth: 0 },
          back: { ...DEFAULT_BIN_PARAMS.walls.back, width: 0, depth: 0 },
          left: { ...DEFAULT_BIN_PARAMS.walls.left, width: 0, depth: 0 },
          right: { ...DEFAULT_BIN_PARAMS.walls.right, width: 0, depth: 0 },
          interior: { ...DEFAULT_BIN_PARAMS.walls.interior, width: 0, depth: 0 },
        },
        inserts: [],
      };
      const result = describeBin(params);
