...
function validPayload() {
  return {
    ...
    params: {
      ...
      label: { enabled: false, style: 'bracket', depth: 12, width: 100, alignment: 'center' },
...
describe('validateDesignerShare', () => {
  ...
  describe('label tab validation', () => {
    ...
    it('rejects invalid label tab style', () => {
      const payload = validPayload();
      (payload.params.label as Record<string, unknown>).style = 'invalid';
      const result = validateDesignerShare(payload, JSON.stringify(payload).length);
      expect(result.valid).toBe(false);
    });

    it('accepts bracket label tab style', () => {
      const payload = validPayload();
      (payload.params.label as Record<string, unknown>).style = 'bracket';
      const result = validateDesignerShare(payload, JSON.stringify(payload).length);
      expect(result.valid).toBe(true);
    });

    it('accepts solid label tab style', () => {
      const payload = validPayload();
      (payload.params.label as Record<string, unknown>).style = 'solid';
      const result = validateDesignerShare(payload, JSON.stringify(payload).length);
      expect(result.valid).toBe(true);
    });
  });
