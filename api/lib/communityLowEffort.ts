/**
 * Server-authoritative low-effort submission predicates for community
 * publishing.
 *
 * MIRROR: duplicates `src/shared/utils/communityLowEffort.ts` (api/ cannot
 * import from src/). Update both sides together; the cross-boundary equality
 * test in `src/shared/utils/communityLowEffort.crossBoundary.test.ts` guards
 * against drift.
 */
