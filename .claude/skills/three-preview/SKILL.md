...
- **The driver owns the puck pose; the app adds only the canvas's limits.** OrbitControls only limits the moves it makes itself, so `cameraCommands.ts` re-imposes the host canvas's `minPolarAngle`/`maxPolarAngle` and a pan leash on the content box. WebHID integrates its own orbit through `constrainPose`, which knows the vertical step it applied and folds a pole crossing back. The driver writes a finished pose through `setViewMatrix`, applied verbatim with the pose's own up on `camera.up` (OrbitControls re-aims on it every frame, and a world-up `lookAt` has no heading at the pole), then `clampElevation` tilts it back about its own right axis and `clampPan` slides it. `endMotion` hands a level pose to the mouse on world up; a rolled one keeps its up.
- **navlib accessors must never throw.** The 3Dconnexion library calls the getters and setters in `spacemouse/navlib/navlibClient.ts` from its WebSocket handler with no try/catch, so a throw leaves the driver's read unanswered and the puck dead for its 2 s timeout. Route every accessor through `guardRead`/`guardCall`, and hit-test solid meshes with `raycaster.camera` set: drei `<Line>` extends `Mesh` and throws without one.
