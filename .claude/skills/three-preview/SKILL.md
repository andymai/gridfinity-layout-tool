...
- **Puck poses are limited by `constrainPose`, not by OrbitControls.** OrbitControls only limits the moves it makes itself, so `cameraCommands.ts` re-imposes the host canvas's `minPolarAngle`/`maxPolarAngle` (else `MIN_POLAR`) and a pan leash on the content box for both transports: WebHID integrates its own orbit, and the driver writes a finished pose through `setViewMatrix`. A pose that crossed a pole arrives folded, polar angle mirrored and azimuth a half turn out, so it is unfolded before clamping or the stall faces backwards.
