...
5. SERP guards in `build-content.ts` are **warn-only**: `SERP_TITLE_MAX_PX = 580` (`scripts/serpTitle.ts`, Arial-width estimate) guards the title, and `composeSerpTitle` appends `" | Gridfinity Layout Tool"` only when it still fits; `MAX_DESCRIPTION_LEN = 155`. Missing `title`/`description` frontmatter is a hard `process.exit(1)`; so is `softwareApplication` without `name` + `applicationCategory`.
...
| Symptom                                                                         | Cause                                                                                                                        | Fix                                                                                                                   |
| ------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Content edit merged to main, production unchanged                               | `scripts/vercel-ignore.sh` deploys main only on release merges or `[deploy]` commits                                         | Wait for the next scheduled release, or put `[deploy]` in the squash commit — see the **release-and-ci** skill        |
