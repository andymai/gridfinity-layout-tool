...
| green tests, user-reported silent no-op / data loss              | a store/hook mock hardcodes the happy value (`baseplateParams: fakeParams`), so the `undefined`/error/fallback branch the bug lives in is unreachable in the test (#2591/#2599, #2444) | make the mocked field settable per test and cover the empty/undefined/error branch; prefer real stores + `resetAllStores()` over hand-mocked state so defaults flow as they do in prod |
| control changed, output/preview identical                        | param dropped between UI and generator, or omitted from the cache key so two values collide (#2554/#2555, #2384)                                                                       | differential test: assert output AND cache key differ across two param values — see geometry-debugging "Assert a parameter actually changes the output"                                |
