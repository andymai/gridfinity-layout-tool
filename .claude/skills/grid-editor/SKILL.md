...
5. **Collision is 3D.** Footprint overlap AND vertical overlap, where a bin's zEnd can protrude past its layer into layers above (blocked zones). zEnd charges `height` plus the larger of `clearanceHeight` and the linked design's real excess rise (`linkedStackExcessUnits`, resolver registered by `LinkedRiseRegistration`). Layer height is only a default for new bins; `canPlaceBin` (`src/shared/utils/validationPlacement.ts`) checks height against `drawer.height - layerZStart`, never against layer height.
