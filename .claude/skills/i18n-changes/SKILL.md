...
- The translated locale JSONs (cs, de, es, fr, it, ja, ko, nb, nl, pl, pt-BR, sv, uk, zh-CN — 14 today) must each have **exactly** the same key set as en.ts. Never trust a memorized count: both check scripts `readdirSync` the locales dir and skip `en.json`, so a new locale is picked up the moment its JSON lands. `SUPPORTED_LOCALES` in `src/i18n/types.ts` (15 incl. en) is the matching runtime list.
...
2. Add the same key, translated, to every non-English locale JSON. If a locale's value is legitimately identical to English (brand name, format string, domain term), add the key to the `"keys"` array in `scripts/i18n-values-allowlist.json`. Identical values ≤3 chars without spaces (`OK`, `3MF`) or matching a `valuePatterns` regex (placeholder-only strings) auto-pass.
...
2. Change en.ts, every non-English JSON, and every code ref in the same commit. Touching only en.ts produces "Missing" (add) or "Extra ... (not in en.ts)" (remove) per locale.
