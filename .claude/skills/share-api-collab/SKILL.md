...
| POST 409 "share with this ID already exists"               | A share already holds the layout's id and this device has no `entry.cloudShare` for it (with one, `useCloudShare` PUTs instead)                                                      | `createShare` retries once under a fresh id; a second 409 surfaces. Never flip `allowOverwrite`                                                                                                                                             |
