...
3. Add a setter in `src/features/bin-designer/store/slices/paramSlice/` that calls `pushHistoryEntry(state)` before mutating (`{ affectsGeometry: false }` only if the worker never reads it).
...
2. Mirror it as a store action in `paramSlice/`. Note: `paramSlice` inlines the algorithm rather than calling the pure utils — a behavior change must land in BOTH copies or store and model diverge.
...
| Symptom                                                           | Cause                                                                                              | Fix                                                                          |
| ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| Labels/divider tilts jump to wrong compartments after merge/split | Remap from `normalizeIdsWithRemap` not threaded through a parallel array                           | Remap every array in the same `set()`; copy `paramSlice/` `mergeCells`       |
...
| Preview never updates (or regenerates on lock/hide clicks)        | Setter skipped `pushHistoryEntry`, or cosmetic prop defaulted to `affectsGeometry: true`           | Fix the setter in `paramSlice/`; see `store/helpers.ts`                      |
...
compartment's X extent must go through `compartmentTabXSpan()` (`utils/compartmentTabFit.ts`),
