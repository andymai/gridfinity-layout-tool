...
**Raw `<button>` / `<select>` / checkbox blocked** — `scripts/check-design-system-usage.ts` (parses the JSX; the old shell version matched line by line and could not see a checkbox whose tag and `type` Prettier had split across lines). Import `{ Button, IconButton, Select, Input, Checkbox }` from `@/design-system`. Text `<input>` is reported but does not block, pending a ~50-site migration. `scripts/design-system-allowlist.txt` exists for temporary exceptions but is empty — migration is complete; do not repopulate it casually.
...
- **A store/hook branch guarded by optional/undefined state** → cover the empty/undefined/error branch with a settable mock or a real store; a mock that hardcodes the happy value hides the exact branch the bug lives in (#2591/#2599).
