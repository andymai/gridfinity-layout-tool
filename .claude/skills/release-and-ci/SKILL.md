...
3. Vercel deploys main **only on release merges or a `[deploy]` commit** (`scripts/vercel-ignore.sh`) — merges accumulate and ship with the next scheduled release, so production deploys ≤3x/day. To ship sooner: trigger the Release workflow manually, or put `[deploy]` in the squash commit message (that is also the route for a `vercel.json`-only change, which bumps no release).
