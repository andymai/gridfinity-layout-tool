...
2. On a schedule (06:00/13:00/23:00 UTC, chosen as activity troughs, or manual `workflow_dispatch`), `.github/workflows/release.yml` runs release-please, which maintains a `chore(main): release gridfinity-layout-tool x.y.z` PR and auto-approves + auto-merges it (GitHub App token, 6 merge retries). No human touches releases. `CHANGELOG.md`, `.release-please-manifest.json`, and the `version` in `package.json` are machine-owned — never hand-edit; the manifest and package.json version must move in lockstep.
3. Vercel deploys main **only on release merges or a `[deploy]` commit** (`scripts/vercel-ignore.sh`) — merges accumulate and ship with the next scheduled release, so production deploys ≤3x/day. That cadence is for CHANGELOG and What's New coherence, not user protection: a deploy never interrupts a live tab (`skipWaiting` and `clientsClaim` both false, `usePWAUpdate` polls hourly behind a smoke gate), and Vercel Skew Protection is enabled, which is a dashboard setting no repo grep will surface. Slot placement is insensitive, landing within 2% of the best available triple in either DST offset, so it needs no DST revisit and is not worth re-litigating. To ship sooner: trigger the Release workflow manually, or put `[deploy]` in the squash commit message (that is also the route for a `vercel.json`-only change, which bumps no release).
...
5. CI is stricter on main than on PRs: PRs run 6 sharded test jobs (3 `generators`, 3 `core`); main runs the full suite **with coverage thresholds** plus the fail-closed OSV scan. A PR can merge green and still break main.
...
| Deploy didn't happen after merging                          | `scripts/vercel-ignore.sh` builds main only on release merges or `[deploy]` commits         | Wait for the next scheduled release, run the Release workflow manually, or use `[deploy]`               |
...
| Unminified production stack traces in PostHog               | Maps upload from the Vercel production build (`scripts/vercel-build.sh`), never from CI     | Check the deploy log; needs POSTHOG_CLI_API_KEY / POSTHOG_CLI_PROJECT_ID in Vercel                      |
